...
EPOCH_MIN = 1 # epoca maxima
EPOCH_MAX = 2 #epoca minima
QTD_TRIALS = 3 # quantidade de tentativas
QTD_SPLITS = 2 # quantidade de folds para usar
...
# ========================================================
BASE_DIR = '/home/aluno-pbarroso/pytorch-pbarroso/FT_YOLO_TRAIN'
CV_DIR = os.path.join(BASE_DIR, 'CV')
arquivo_yolo_yaml = os.path.join(BASE_DIR, 'yolo.yaml')
...
if __name__ == "__main__":
    ...
    # Executa QTD trials com modelos aleatórios
    for i in range(QTD_TRIALS):
        model_name = random.choice(list(MODELS.keys()))
        trial = study.ask()
    ...
    print(f"Melhor média ponderada: {best_params}")

    logger.info("Melhor modelo global: %s", best_model_name)
    logger.info("Melhor média ponderada: %s", best_params)
